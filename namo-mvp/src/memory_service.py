...
class MemoryService:
    def __init__(self):
        ...
        # This prevents iterating through all emotion groups on every check.
        self.emotion_to_group_map = self._create_emotion_map()

    def _create_emotion_map(self) -> Dict[str, str]:
        """Creates a reverse map from an emotion to its group for O(1) lookups."""
        emotion_groups = {
    ...
    def _is_related_emotion(self, emotion1: str, emotion2: str) -> bool:
        """
        Checks if two emotions belong to the same group using a pre-computed map.
        Time complexity: O(1) on average.
        """
        e1_lower = emotion1.lower()
        e2_lower = emotion2.lower()

        group1 = self.emotion_to_group_map.get(e1_lower)
        group2 = self.emotion_to_group_map.get(e2_lower)
